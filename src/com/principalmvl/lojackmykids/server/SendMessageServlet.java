...
public class SendMessageServlet extends BaseServlet {
	...
	static final String PARAMETER_DEVICE = "registration_ids";
	...
	private void sendSingleMessage(String regId, HttpServletResponse resp) {
		...
		if (result.getMessageId() != null) {
			...
			if (canonicalRegId != null) {
				...
				DatastoreServerHelper.updateRegistration(regId, canonicalRegId);
			}
		} else {
			String error = result.getErrorCodeName();
			if (error.equals(Constants.ERROR_NOT_REGISTERED)) {
				// application has been removed from device - unregister it
				DatastoreServerHelper.unregister(regId);
			} else {
	...
	private void sendMulticastMessage(String multicastKey,
		...
		List<String> regIds = DatastoreServerHelper.getMulticast(multicastKey);
		Message message = createMessage();
		...
		if (multicastResult.getFailure() != 0) {
			...
			if (!retriableRegIds.isEmpty()) {
				// update task
				DatastoreServerHelper.updateMulticast(multicastKey, retriableRegIds);
				allDone = false;
				retryTask(resp);
			}
		}
	...
	private void multicastDone(HttpServletResponse resp, String encodedKey) {
		DatastoreServerHelper.deleteMulticast(encodedKey);
		taskDone(resp);
	}
